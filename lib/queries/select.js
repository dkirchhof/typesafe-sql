...
class SelectQuery {
    ...
    getOne(resultSetFactory) {
        ...
        return new ExecutableSelectQuery(selectedColumns, this.source, this.joins, this.wheres, this.havings, this.groupBys, this.orderBys, this.isDistinct, 1);
    }
    getMany(resultSetFactory) {
        const resultSetSchema = resultSetFactory(this.record);
        const selectedColumns = Object.entries(resultSetSchema).map(([alias, column]) => new Projection_1.Projection(column, alias));

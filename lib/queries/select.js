...
class SelectQuery {
    constructor(table, alias) {
        ...
        this.groupBys = [];
        this.orderBys = [];
        this.source = new Source_1.Source(table, alias);
        this.updateRecord(table, alias);
    }
    join(joinMode, table, alias, predicateFactory) {
        this.updateRecord(table, alias);
        const predicate = predicateFactory(this.record);
        this.joins.push(new Join_1.Join(joinMode, new Source_1.Source(table, alias), predicate));
        return this;
    }
    ...
    updateRecord(table, alias) {
        this.record[alias] =
            Object.keys(table.columns)
                .reduce((prev, column) => ({ ...prev, [column]: new Column_1.Column(alias, column) }), {});
    }
}

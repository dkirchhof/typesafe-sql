...
class ExecutableUpdateQuery {
    constructor(table, values) {
        ...
        this.columns = Object.keys(table.columns)
            .reduce((prev, columnName) => ({ ...prev, [columnName]: new Column_1.Column(columnName) }), {});
    }
    ...
    setValuesToSQL() {
        const values = Object.entries(this.values).map(([columnName, value]) => `${columnName} = ${utils_1.sanitizeValue(value)}`).join(", ");
        return `SET ${values}`;
    }
    wheresToSQL() {
